...
English | [中文](snapshot-format.zh.md)

The contract for the observation pipeline: the JSON the in-page
serializer produces, how references are minted, the text rendering
agents read, and the token-budget rules. `rutter-observe` implements
exactly this; golden and property tests pin it
([testing](testing.md)).
...
([`observe::serializer_script()`](../crates/observe/src/lib.rs)). The
...
JSON with `observe::snapshot_from_response()`.
...
guessed at (the honesty rule). The serializer never throws: every
per-node computation is guarded, and on failure the node degrades to
role `generic` with no name.
...
characters, names and values to 200) and set `truncated`. The
converter never fails on hostile page data — the
[hostile-input discipline](architecture.md#cross-cutting-invariants)
...
Budget is measured in rendered characters of the YAML text; the
default budget is 20 000 characters per snapshot. Rules apply in
order; every budget-induced change sets `Snapshot::truncated = true`.

1. **Depth budget.** Nodes deeper than 48 levels below the root are
...
3. **Viewport-first culling.** Node rects are viewport-relative; any
   ...
   size exceeds 400 characters is replaced by a summary node of its
   root (`× N` counts the folded children), regardless of the
   remaining budget.
...
5. **Safety caps** (hostile-input limits, independent of budget):
   depth 512, 100 000 nodes; tripping them sets `truncated` and
