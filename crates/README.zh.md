...
合根）直接依赖除 `events` 外的全部成员（`events` 经 `session` 与
`dashboard` 间接到达）。这些接缝的存在让每个 crate 只有一个变更理由
...
| [`events/`](events/README.md) | 类型化 Event（事件）骨干：bus、每会话 ring buffer（环形缓冲）、重放 | core, policy |
...
| [`mcp/`](mcp/README.md) | 基于 rmcp 的 MCP 工具面（stdio + streamable HTTP） | core, session（engine、policy 仅为 dev-dependency） |
| [`dashboard/`](dashboard/README.md) | 本地监督面板（事件、审批、screencast（屏幕流）） | core, events, policy, session（engine 仅为 dev-dependency） |
| [`cli/`](cli/README.md) | 二进制入口模式：browse、serve、open；组装一切 | 以上除 `events` 外的全部 |
