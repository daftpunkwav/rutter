//! Shared fixtures for the MCP transport tests: a scriptable engine
//! double chain (launcher → engine → context → page) answering the
//! observe-script markers without a browser.
...
impl Default for PageInner {
    fn default() -> Self {
        Self {
            ...
            found: AtomicBool::new(true),
        }
    }
}

/// A page double answering the observe-script markers: serializer
/// envelope, resolver box, text-wait flag, and `location.href`.
...
impl PageHandle for FlowPage {
    ...
    async fn evaluate(&self, expression: &str) -> Result<Value, EngineError> {
        if expression.contains("var NEEDLE = ") {
            Ok(json!({
                "found": self.inner.found.load(Ordering::SeqCst)
            }))
...
struct ContextInner {
    id: ContextId,
    pages: Mutex<Vec<(PageId, Arc<FlowPage>)>>,
    counter: AtomicU64,
    closed: AtomicBool,
}
